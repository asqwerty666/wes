...
Data PATHS
"""
src_dir = '/nas/osotolongo/wes/store/crams'
...
output_dir = '/nas/osotolongo/wes/store/output'
...
dir_cont = next(os.walk(src_dir))[2]

if cfile and os.path.isfile(cfile):
	f = open(cfile, 'r')
	cuts = f.read().splitlines()
	dir_cont = list(filter(lambda x: x+'.cram' in set(dir_cont), set(cuts)))
...
for pollo in dir_cont:
    ...
    ljobids = []
    call = [samtools, 'view', '-H', src_dir + '/' + pollo +'.cram']
    ...
    cdata['command'] += samtools + ' view -b -o ' + tmp_dir + '/' + pollo +'.bam ' + src_dir + '/' + pollo +'.cram\n'
